...
// Alterações realizadas em relação ao arquivo original:
...
void devolucao(cliente *pcli, int qcli, carro *pcar);  //sem int qcar
...
int main(){
	...
	do{
		...
		switch(op){
			...
			case 2: devolucao(cli, cont, car);			//chamada da funcao devolucao e retirada do parametro int qcar
					break;
		}//switch
